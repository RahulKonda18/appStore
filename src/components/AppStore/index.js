...
class AppStore extends Component {
  state = {tab: tabsList[0].tabId, search: ''}

  changeSearch = event => this.setState({search: event.target.value})

  tabChange = id => this.setState({tab: id})

  render() {
    const {tab, search} = this.state
    const resultsList = appsList.filter(each => each.category === tab)
    const x = resultsList.filter(each =>
      each.appName.toLowerCase().includes(search.toLowerCase()),
    )
    return (
      <div className="bg">
